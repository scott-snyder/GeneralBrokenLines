 ...
 */
GblHelixPrediction* GblSimpleHelix::getPredictionPtr(const Eigen::Vector3d& refPos,
...
// projections on measurement directions
	...
	return new GblHelixPrediction(sArc2D, pred, tDir, uDir, vDir, nDir, pos);
}
...
GblDetectorLayer::GblDetectorLayer(const std::string aName,
		...
		const Eigen::Vector3d& aCenter, const Eigen::Vector2d& aResolution,
		const Eigen::Vector2d& aPrecision, const Eigen::Matrix3d& measTrafo,
		const Eigen::Matrix3d& alignTrafo) :
